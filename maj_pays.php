...
if(isset($_POST["nom_pays"])){
	$nom_pays = pg_escape_string($_POST["nom_pays"]);	
}
...
if (isset($_GET['type_modif']))
{
	if ($_GET['type_modif'] == 'supp' && $id_rech !=""){				
		pg_query("DELETE FROM pays WHERE id=$id_rech");
	}
	if($_GET['type_modif'] == 'ajout'){
		if($nom_pays != "" && $ordre) {				
			$result = pg_query ($dbconn, "INSERT INTO pays (libelle, ordre) 
										  VALUES ('".$nom_pays ."', '".$ordre."') RETURNING id");
	...
	if ($_GET['type_modif'] == 'modif'){
		if($nom_pays != "" && $ordre) {		
			pg_query ($dbconn, "UPDATE pays SET libelle='".$nom_pays."', ordre='".$ordre. "'
							WHERE id=".$id_rech."");	
		}	
	}
	pg_query ($dbconn, "INSERT INTO historique (login, type, objet, heure, timestamp2, id_objet) 
							VALUES ('".$_SESSION['login']."', '".$_GET['type_modif']."', 'pays', '". $today ."' ,  now()  , '". $id_rech ."' )");	
}
